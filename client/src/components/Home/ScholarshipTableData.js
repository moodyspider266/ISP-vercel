export const ScholarshipTableData = {
    tab1Card1: {
      title: "Department of Empowerment of Persons with Disabilities",
      table: [
        [
          "Scheme Name",
          "Scheme Closing Date",
          "Defective Application Verification Date",
          "Institute Verification",
          "DNO/SNO/MNO Verification",
          "Guidelines/FAQ",
        ],
        [
          "Pre-matric Scholarship for Students with Disabilities",
          "Open till 15-01-2024",
          "Open till 31-01-2024",
          "Open till 31-01-2024",
          "Open till 2024-02-10",
          "Guidelines/FAQ",
        ],
        [
          "Post-matric Scholarship for Students with Disabilities",
          "Open till 15-01-2024",
          "Open till 31-01-2024",
          "Open till 31-01-2024",
          "Open till 2024-02-10	",
          "Guidelines/FAQ",
        ],
        // More rows as needed
      ],
    },
    tab1Card2: {
      title: "Ministry of Minority Affairs",
      table: [
        [
          "Scheme Name",
          "Scheme Closing Date",
          "Defective Application Verification Date",
          "Institute Verification",
          "DNO/SNO/MNO Verification",
          "Guidelines/FAQ",
        ],
        [
          "Pre-matric Scholarship for Students with Disabilities",
          "Open till 15-01-2024",
          "Open till 31-01-2024",
          "Open till 31-01-2024",
          "Open till 2024-02-10",
          "Guidelines/FAQ",
        ],
        // More rows as needed
      ],
    },
    tab1Card3: {
      title: "Ministry of Social Justice & Empowerment",
      table: [
        [
          "Scheme Name",
          "Scheme Closing Date",
          "Defective Application Verification Date",
          "Institute Verification",
          "DNO/SNO/MNO Verification",
          "Guidelines/FAQ",
        ],
        [
          "Pre-matric Scholarship for Students with Disabilities",
          "Open till 15-01-2024",
          "Open till 31-01-2024",
          "Open till 31-01-2024",
          "Open till 2024-02-10",
          "Guidelines/FAQ",
        ],
        [
          "Post-matric Scholarship for Students with Disabilities",
          "Open till 15-01-2024",
          "Open till 31-01-2024",
          "Open till 31-01-2024",
          "Open till 2024-02-10	",
          "Guidelines/FAQ",
        ],
        // More rows as needed
      ],
    },
    tab1Card4: {
      title: "Ministry of Labour & Employment",
      table: [
        [
          "Scheme Name",
          "Scheme Closing Date",
          "Defective Application Verification Date",
          "Institute Verification",
          "DNO/SNO/MNO Verification",
          "Guidelines/FAQ",
        ],
        [
          "Pre-matric Scholarship for Students with Disabilities",
          "Open till 15-01-2024",
          "Open till 31-01-2024",
          "Open till 31-01-2024",
          "Open till 2024-02-10",
          "Guidelines/FAQ",
        ],
        [
          "Post-matric Scholarship for Students with Disabilities",
          "Open till 15-01-2024",
          "Open till 31-01-2024",
          "Open till 31-01-2024",
          "Open till 2024-02-10	",
          "Guidelines/FAQ",
        ],
        // More rows as needed
      ],
    },
    tab1Card5: {
      title: "Ministry of Tribal Affairs",
      table: [
        [
          "Scheme Name",
          "Scheme Closing Date",
          "Defective Application Verification Date",
          "Institute Verification",
          "DNO/SNO/MNO Verification",
          "Guidelines/FAQ",
        ],
        [
          "Pre-matric Scholarship for Students with Disabilities",
          "Open till 15-01-2024",
          "Open till 31-01-2024",
          "Open till 31-01-2024",
          "Open till 2024-02-10",
          "Guidelines/FAQ",
        ],
        [
          "Post-matric Scholarship for Students with Disabilities",
          "Open till 15-01-2024",
          "Open till 31-01-2024",
          "Open till 31-01-2024",
          "Open till 2024-02-10	",
          "Guidelines/FAQ",
        ],
        // More rows as needed
      ],
    },
    tab1Card6: {
      title: "Department of School Education & Literacy",
      table: [
        [
          "Scheme Name",
          "Scheme Closing Date",
          "Defective Application Verification Date",
          "Institute Verification",
          "DNO/SNO/MNO Verification",
          "Guidelines/FAQ",
        ],
        [
          "Pre-matric Scholarship for Students with Disabilities",
          "Open till 15-01-2024",
          "Open till 31-01-2024",
          "Open till 31-01-2024",
          "Open till 2024-02-10",
          "Guidelines/FAQ",
        ],
        [
          "Post-matric Scholarship for Students with Disabilities",
          "Open till 15-01-2024",
          "Open till 31-01-2024",
          "Open till 31-01-2024",
          "Open till 2024-02-10	",
          "Guidelines/FAQ",
        ],
        // More rows as needed
      ],
    },
  
    // Data for cards in Tab 2
    tab2Card1: {
      title: "All India Council For Technical Education-AICTE",
      table: [
        ["Column 1", "Column 2", "Column 3", "Column 4", "Column 5", "Column 6"],
        [
          "Row 1 Data 1",
          "Row 1 Data 2",
          "Row 1 Data 3",
          "Row 1 Data 4",
          "Row 1 Data 5",
          "Row 1 Data 6",
        ],
        [
          "Row 2 Data 1",
          "Row 2 Data 2",
          "Row 2 Data 3",
          "Row 2 Data 4",
          "Row 2 Data 5",
          "Row 2 Data 6",
        ],
        // More rows as needed
      ],
    },
    tab2Card2: {
      title: "University Grants Commission - MHRD",
      table: [
        ["Column 1", "Column 2", "Column 3", "Column 4", "Column 5", "Column 6"],
        [
          "Row 1 Data 1",
          "Row 1 Data 2",
          "Row 1 Data 3",
          "Row 1 Data 4",
          "Row 1 Data 5",
          "Row 1 Data 6",
        ],
        // More rows as needed
      ],
    },
    tab3Card1: {
      title: "Maharashtra",
      table: [
        ["Column 1", "Column 2", "Column 3", "Column 4", "Column 5", "Column 6"],
        [
          "Row 1 Data 1",
          "Row 1 Data 2",
          "Row 1 Data 3",
          "Row 1 Data 4",
          "Row 1 Data 5",
          "Row 1 Data 6",
        ],
        // More rows as needed
      ],
    },
    tab3Card2: {
      title: "Delhi",
      table: [
        ["Column 1", "Column 2", "Column 3", "Column 4", "Column 5", "Column 6"],
        [
          "Row 1 Data 1",
          "Row 1 Data 2",
          "Row 1 Data 3",
          "Row 1 Data 4",
          "Row 1 Data 5",
          "Row 1 Data 6",
        ],
        // More rows as needed
      ],
    },
    tab3Card3: {
      title: "Kashmir",
      table: [
        ["Column 1", "Column 2", "Column 3", "Column 4", "Column 5", "Column 6"],
        [
          "Row 1 Data 1",
          "Row 1 Data 2",
          "Row 1 Data 3",
          "Row 1 Data 4",
          "Row 1 Data 5",
          "Row 1 Data 6",
        ],
        // More rows as needed
      ],
    },
    tab4Card1: {
      title: "VESIT Scholarship",
      table: [
        ["Column 1", "Column 2", "Column 3", "Column 4", "Column 5", "Column 6"],
        [
          "Row 1 Data 1",
          "Row 1 Data 2",
          "Row 1 Data 3",
          "Row 1 Data 4",
          "Row 1 Data 5",
          "Row 1 Data 6",
        ],
        // More rows as needed
      ],
    },
    tab4Card2: {
      title: "MU Scholarship",
      table: [
        ["Column 1", "Column 2", "Column 3", "Column 4", "Column 5", "Column 6"],
        [
          "Row 1 Data 1",
          "Row 1 Data 2",
          "Row 1 Data 3",
          "Row 1 Data 4",
          "Row 1 Data 5",
          "Row 1 Data 6",
        ],
        // More rows as needed
      ],
    },
    // Add more cards for other tabs as needed
  };

export default ScholarshipTableData;